...
```sh
go install github.com/omriariav/amq-squad/v2/cmd/amq-squad@v2.11.0
amq-squad version
```
...
amq-squad v2.11.0 requires AMQ 0.38.0 or newer. That floor includes eval-safe `amq env --export` and the reserved human `user` mailbox behavior used by operator gates and notification surfaces.
...
amq-squad v2.11.0 requires amq **0.38.0+**. Launches pass `--require-wake` to
