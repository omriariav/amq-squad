...
// machine-readable read outputs (status, history, team show / up --dry-run,
// version, team profiles). Bump only on a breaking change to the envelope
// shape; field additions inside `data` do not require a bump.
const JSONSchemaVersion = 1

// jsonEnvelope is the wrapping shape every --json output uses. Kind names
// the produced view (e.g. "status", "history", "team_plan", "version",
// "team_profiles", "brief_candidate"); Data is the kind-specific payload.
// Stdout receives the envelope alone; diagnostics stay on stderr.
