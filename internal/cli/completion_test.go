...
func TestRunCompletionBashContainsRepresentativeTokens(t *testing.T) {
	...
	for _, want := range []string{
		...
		"goal_subcommands", "apply", "deliver", "draft", "start",
...
func TestRunCompletionFishContainsRepresentativeTokens(t *testing.T) {
	...
	for _, want := range []string{
		...
		"-a 'apply'", "-a 'deliver'", "-a 'draft'", "-a 'start'",
		"__fish_seen_subcommand_from operator",
		"-a 'answer'", "-a 'directive'", "-a 'poll'", "-a 'watch'",
		"__fish_seen_subcommand_from rules",
		"-a 'show'",
		"-l 'profile'", "-l 'json'", "-l 'actions'", "-l 'action'", "-l 'action-id'", "-l 'target-id'", "-l 'scope'", "-l 'run-action'", "-l 'set'", "-l 'commands'", "-l 'mutating'", "-l 'dry-run'", "-l 'force-duplicate'", "-l 'approved'", "-l 'denied'", "-l 'gate'", "-l 'goal-id'",
