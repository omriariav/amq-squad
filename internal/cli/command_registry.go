...
var commandCatalog = []struct {
	...
	{Name: "brief", Summary: "Draft or seed the workstream brief (the only drafter on any launch-capable path)"},
	...
	{Name: "wizard", Summary: "Deterministic goal-to-squad setup with one default-No launch review"},
	{Name: "start", Summary: "Reconcile and launch a team's canonical workstream"},
	{Name: "plan", Summary: "Zero-write preview of a team launch via launchapi.Prepare"},
...
func commandRegistry(version string) []commandMeta {
	return []commandMeta{
		...
		{Name: "wizard", Summary: commandSummary("wizard"), Run: func(args []string) error { return runWizardWithVersion(args, version) }},
		{Name: "start", Summary: commandSummary("start"), Run: runStart},
		{Name: "plan", Summary: commandSummary("plan"), Run: runPlan},
